...
const STATIC_USER = { email: 'karim@admin.com', password: '12345' }
...
export default function LoginForm({ onSwitch }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    setError('')
    if (email.toLowerCase() === STATIC_USER.email && password === STATIC_USER.password) {
      localStorage.setItem('token', 'static-auth-token')
      localStorage.setItem('user', JSON.stringify({ email: STATIC_USER.email, name: 'Karim' }))
      navigate('/')
    } else {
    ...
    <div className="max-w-sm w-full mx-auto md:mx-0">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-4 pt-4">
          <button
            type="submit"
            className="w-full bg-primary text-on-primary py-4 font-label text-xs tracking-[0.15em] uppercase font-bold hover:bg-zinc-800 active:scale-[0.98] transition-colors"
          >
            Se connecter
          </button>
