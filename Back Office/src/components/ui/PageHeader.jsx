...
PageHeader.SecondaryBtn = function SecondaryBtn({ icon, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className="px-4 py-2.5 bg-slate-100 text-slate-700 font-semibold rounded-xl text-sm flex items-center gap-2 hover:bg-slate-200 transition-all border border-slate-200"
    >
...
PageHeader.DangerBtn = function DangerBtn({ icon, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className="px-4 py-2.5 border border-red-200 text-red-600 font-semibold rounded-xl text-sm flex items-center gap-2 hover:bg-red-50 transition-all"
    >
      {icon && <span className="material-symbols-outlined text-lg">{icon}</span>}
      {children}
    </button>
